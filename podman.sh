...
start_container() {
    echo -e "${YELLOW}Starting Family Dinner Planner with Podman...${NC}"
    ...
    podman build -t dinner-planner:latest .
    
    # Run the container
    echo "Starting the container..."
    podman run --name ${CONTAINER_NAME} -d -p 5000:5000 -v "$(pwd):/app:Z" dinner-planner:latest
...
stop_container() {
    echo -e "${YELLOW}Stopping Family Dinner Planner...${NC}"
...
show_logs() {
    echo -e "${YELLOW}Showing logs for Family Dinner Planner...${NC}"
...
show_help() {
    echo -e "${GREEN}Family Dinner Planner - Podman Script${NC}"
    echo ""
